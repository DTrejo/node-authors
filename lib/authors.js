...
function lookupGithubLogin (p, print, callback) {
  ...
  if (process.env.OAUTH_TOKEN) {
    options['Authorization'] = 'token ' + process.env.OAUTH_TOKEN.trim()
  }
  function cb (err, p) {
    callback(err, p)
  }
  ...
  nameAndEmail(function (er, ppl) {
    ...
    async.parallel(tasks, function (err, results) {
      ...
      if (rateLimitExceeded) {
        log('\nWarning: GitHub API rate limit exceeded! The result might be incomplete! You can provide an OAUTH_TOKEN to increase your limit.')
      }
      if (print) {
        log('')
        log('## Contributors')
